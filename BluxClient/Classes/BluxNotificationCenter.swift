...
@objc public class BluxNotificationCenter: NSObject, UNUserNotificationCenterDelegate {
    @objc static public let shared = BluxNotificationCenter()
    ...
    @objc public func userNotificationCenter(_ center: UNUserNotificationCenter, didReceive response: UNNotificationResponse, withCompletionHandler completionHandler: @escaping () -> Void) {
        ...
        if let notification = BluxNotification.getBluxNotificationFromUNNotificationContent(response.notification.request.content) {
            ...
            if let bluxDismissLaunchUrl = Bundle.main.object(forInfoDictionaryKey: "blux_dismiss_launch_url") as? Bool, bluxDismissLaunchUrl == true {
                Logger.verbose("Launch url was dismissed because blux_dismiss_launch_url in Info.plist is YES.")
                return
            }
            
            if let bluxDismissLaunchUrl = notification.data?["blux_dismiss_launch_url"] as? String, bluxDismissLaunchUrl == "true" {
                Logger.verbose("Launch url was dismissed because blux_dismiss_launch_url is true.")
                return
            }
    ...
    @objc public func userNotificationCenter(_ center: UNUserNotificationCenter, willPresent notification: UNNotification, withCompletionHandler completionHandler: @escaping (UNNotificationPresentationOptions) -> Void) {
        if let bluxNotification = BluxNotification.getBluxNotificationFromUNNotificationContent(notification.request.content) {
            if let bluxDismissForegroundNotification = bluxNotification.data?["blux_dismiss_foreground_notification"] as? String,
               bluxDismissForegroundNotification == "true" {
                Logger.verbose("Foreground notification received was dismissed because blux_dismiss_foreground_notification is true.")
                return
            }
            
            let event = NotificationReceivedEvent(UIApplication.shared, notification: bluxNotification, completionHandler: completionHandler)
            
            if let handler = EventHandlers.notificationForegroundReceived {
                Logger.verbose("Handle foreground notification received with registered handler.")
                handler(event)
            } else {
                event.display()
            }
